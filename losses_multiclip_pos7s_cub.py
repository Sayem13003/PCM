...
def loss_EM_PL(batch, P, Z):
    # unpack:
    preds = batch['preds']
    #print('predss_shape',preds.shape)
    #print("preds: ", preds)
    observed_labels = batch['label_vec_obs']
    observed_labels2=observed_labels.repeat(7, 1)
    num_classes = int(batch['label_vec_obs'].size(1))
    batch_size = int(batch['label_vec_obs'].size(0))
    #preds = preds.reshape(batch_size, 3, num_classes).transpose(1, 0, 2).reshape(batch_size, num_classes)
    preds = preds.reshape(batch_size, 7, num_classes).permute(1, 0, 2).reshape(batch_size*7, num_classes)
    ...
    if P['epoch'] > P['warmup_epoch']:
        ...
        final_labels = torch.where(observed_labels2 == 0, pseudo_labels, observed_labels2)
        #print("final_labels: ", final_labels)
        #input validation:
        assert torch.min(final_labels) >= -1
        
        loss_mtx = torch.zeros_like(preds)
        #print('loss_mtx_shape',loss_mtx.shape)
        #print('observed_labels_shape',observed_labels.shape)
        #####
        #observed positive label
        #####
        loss_mtx[observed_labels2 == 1] = neg_log(preds[observed_labels2 == 1]) #+ neg_log(similarity[observed_labels == 1])
        ...
        mask_pos = (observed_labels2  == 0) & (pseudo_labels == 1)
        #print(mask)
        ...
        #negative pseudo-label 
        #mask_neg = (observed_labels2  == 0) & (pseudo_labels ==-1)
...
def compute_batch_loss(batch, P, Z):
    #print("Shape of batch['preds']:", batch['preds'].shape)

    assert batch['preds'].dim() == 2
    preds=batch['preds']
    batch_size = int(batch['preds'].size(0))
    num_classes = int(batch['preds'].size(1)/7)
